...
class MultiscaleNet(nn.Module):
    def __init__(self,
        ...
        self.patch_embed_mid_INR2 = OverlapPatchEmbed(inp_channels, dim)

        self.encoder_level1_mid1 = nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
        self.encoder_level1_mid1_INR2 = nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
        ...
        self.down1_2_mid_INR2 = Downsample(dim)
        self.down1_2_mid2 = Downsample(dim)

        self.encoder_level2_mid1 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])
        self.encoder_level2_mid1_INR2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])
        ...
        self.down2_3_mid_INR2 = Downsample(int(dim * 2 ** 1))
        self.down2_3_mid2 = Downsample(int(dim * 2 ** 1))
        self.latent_mid1 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])
        self.latent_mid1_INR2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])
    ...
    def forward(self, inp_img):
        ...
        ###############################################################################################
        inp_enc_level1_small_INR1 = self.patch_embed_small_INR1(inp_img_small)
        out_enc_level1_small_INR1 = self.encoder_level1_small_INR1(inp_enc_level1_small_INR1)
        inp_enc_level2_small_INR1 = self.down1_2_small_INR1(out_enc_level1_small_INR1)
        out_enc_level2_small_INR1 = self.encoder_level2_small_INR1(inp_enc_level2_small_INR1)
        inp_enc_level4_small_INR1 = self.down2_3_small_INR1(out_enc_level2_small_INR1)
        latent_small_INR1 = self.latent_small_INR1(inp_enc_level4_small_INR1)
        latent_small_mid_INR1 = self.upsmall2mid1(latent_small_INR1)
        latent_small_mid_INR1 = self.upsmall2mid2(latent_small_mid_INR1)
        ###############################################################################################

        outputs.append(inp_img_small)
        INR = self.INR(latent_small_mid_INR1)
        ...
        ############################################################################################
        inp_enc_level1_mid_INR2 = self.patch_embed_mid_INR2(mid_img)
        out_enc_level1_mid_INR2 = self.encoder_level1_mid1_INR2(inp_enc_level1_mid_INR2)

        inp_enc_level2_mid_INR2 = self.down1_2_mid_INR2(out_enc_level1_mid_INR2)
        out_enc_level2_mid_INR2 = self.encoder_level2_mid1_INR2(inp_enc_level2_mid_INR2)

        inp_enc_level4_mid_INR2 = self.down2_3_mid_INR2(out_enc_level2_mid_INR2)
        latent_mid_INR2 = self.latent_mid1_INR2(inp_enc_level4_mid_INR2)
        latent_mid_INR_max_INR2 = self.upmid2max1(latent_mid_INR2)
        latent_mid_INR_max_INR2 = self.upmid2max2(latent_mid_INR_max_INR2)

        ############################################################################################

        outputs.append(mid_img / 2)
        INR2 = self.INR2(latent_mid_INR_max_INR2)
