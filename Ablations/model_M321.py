...
class MultiscaleNet(nn.Module):
    def __init__(self,
        ...
        #############################################################################################################################
        self.encoder_level1_small2 = nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])

        self.down1_2_small2 = Downsample(dim)
        self.encoder_level2_small2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.down2_3_small2 = Downsample(int(dim * 2 ** 1))
        self.latent_small2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])

        self.up3_2_small2 = Upsample(int(dim * 2 ** 2))
        self.reduce_chan_level2_small2 = nn.Conv2d(int(dim * 2 ** 2), int(dim * 2 ** 1), kernel_size=1, bias=bias)
        self.decoder_level2_small2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.up2_1_small2 = Upsample(int(dim * 2 ** 1))
        # self.reduce_chan_level1_small2 = nn.Conv2d(int(dim * 2 ** 1), int(dim * 1 ** 1), kernel_size=1, bias=bias)
        self.decoder_level1_small2 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
        #########
        self.encoder_level1_small3 = nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                             LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])

        self.down1_2_small3 = Downsample(dim)
        self.encoder_level2_small3 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.down2_3_small3 = Downsample(int(dim * 2 ** 1))
        self.latent_small3 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])

        self.up3_2_small3 = Upsample(int(dim * 2 ** 2))
        self.reduce_chan_level2_small3 = nn.Conv2d(int(dim * 2 ** 2), int(dim * 2 ** 1), kernel_size=1, bias=bias)
        self.decoder_level2_small3 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[1])])

        self.up2_1_small3 = Upsample(int(dim * 2 ** 1))
        # self.reduce_chan_level1_small3 = nn.Conv2d(int(dim * 2 ** 1), int(dim * 1 ** 1), kernel_size=1, bias=bias)
        self.decoder_level1_small3 = nn.Sequential(*[
            TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[0])])
        #########
        self.output_small_context = nn.Conv2d(int(dim * 2 ** 1), dim, kernel_size=3, stride=1, padding=1, bias=bias)
        self.output_small_context2 = nn.Conv2d(int(dim * 2 ** 1), dim, kernel_size=3, stride=1, padding=1, bias=bias)
        #############################################################################################################################
    ...
    def forward(self, inp_img):
        ...
        #######################################################################################################
        inp_enc_level1_small = self.output_small_context(out_dec_level1_small)

        out_enc_level1_small = self.encoder_level1_small2(inp_enc_level1_small)

        inp_enc_level2_small = self.down1_2_small2(out_enc_level1_small)
        out_enc_level2_small = self.encoder_level2_small2(inp_enc_level2_small)

        inp_enc_level4_small = self.down2_3_small2(out_enc_level2_small)
        latent_small = self.latent_small2(inp_enc_level4_small)

        inp_dec_level2_small = self.up3_2_small2(latent_small)
        inp_dec_level2_small = torch.cat([inp_dec_level2_small, out_enc_level2_small], 1)
        inp_dec_level2_small = self.reduce_chan_level2_small2(inp_dec_level2_small)

        out_dec_level2_small = self.decoder_level2_small2(inp_dec_level2_small)

        inp_dec_level1_small = self.up2_1_small2(out_dec_level2_small)
        inp_dec_level1_small = torch.cat([inp_dec_level1_small, out_enc_level1_small], 1)
        # inp_dec_level1_small = self.reduce_chan_level1_small2(inp_dec_level1_small)

        out_dec_level1_small = self.decoder_level1_small2(inp_dec_level1_small)

        ###########################
        inp_enc_level1_small = self.output_small_context2(out_dec_level1_small)

        out_enc_level1_small = self.encoder_level1_small3(inp_enc_level1_small)

        inp_enc_level2_small = self.down1_2_small3(out_enc_level1_small)
        out_enc_level2_small = self.encoder_level2_small3(inp_enc_level2_small)

        inp_enc_level4_small = self.down2_3_small3(out_enc_level2_small)
        latent_small = self.latent_small3(inp_enc_level4_small)

        inp_dec_level2_small = self.up3_2_small3(latent_small)
        inp_dec_level2_small = torch.cat([inp_dec_level2_small, out_enc_level2_small], 1)
        inp_dec_level2_small = self.reduce_chan_level2_small3(inp_dec_level2_small)

        out_dec_level2_small = self.decoder_level2_small3(inp_dec_level2_small)

        inp_dec_level1_small = self.up2_1_small3(out_dec_level2_small)
        inp_dec_level1_small = torch.cat([inp_dec_level1_small, out_enc_level1_small], 1)
        # inp_dec_level1_small = self.reduce_chan_level1_small3(inp_dec_level1_small)

        out_dec_level1_small = self.decoder_level1_small3(inp_dec_level1_small)
        ...
        out_dec_level1_max = self.decoder_level1_max1(inp_dec_level1_max) +mid_2_max
